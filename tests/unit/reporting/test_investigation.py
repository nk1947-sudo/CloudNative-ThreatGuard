...
class TestWorkloadInvestigator(unittest.TestCase):
    ...
    def test_socket_connection_from_metadata_only_event(self):
        """
        Regression guard: a RUNTIME-006 detection from the real detection engine
        carries destination_ip/destination_port/protocol only inside metadata (the
        top-level SecurityEvent.destination_ip field is left empty), because
        evidence dicts map onto metadata, not the top-level dataclass fields.
        investigate() previously looked for the wrong metadata keys (daddr/dport/
        proto) and would report the connection as destination 'unknown'.
        """
        events = [
            SecurityEvent(
                event_id="ev-net",
        ...
        self.assertEqual(dossier["telemetry_summary"]["socket_connections"], 1)
        conn = dossier["socket_connections"][0]
        self.assertEqual(conn["destination_ip"], "198.51.100.24")
        self.assertEqual(conn["destination_port"], 4444)
