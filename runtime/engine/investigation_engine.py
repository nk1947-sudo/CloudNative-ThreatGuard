...
class WorkloadInvestigator:
    ...
    def investigate(
        ...
        for ev in workload_events:
            ...
            if ev.action in ["process_exec", "detected"] and ev.process:
            ...
            if ev.destination_ip or "connect" in ev.action.lower() or ev.mitre_tactic == "Command and Control":
                socket_connections.append({
                    ...
                    "event_id": ev.event_id,
                    "process": ev.process,
                    "destination_ip": ev.destination_ip or ev.metadata.get("daddr", "unknown"),
                    "destination_port": ev.destination_port or ev.metadata.get("dport", 0),
                    "protocol": ev.metadata.get("proto", "TCP"),
                    "direction": "OUTBOUND"
                })
            ...
            if "file" in ev.action.lower() or ev.mitre_tactic == "Credential Access" or ev.file_path:
                file_access_log.append({
                    ...
                    "event_id": ev.event_id,
                    "process": ev.process,
                    "file_path": ev.file_path or ev.metadata.get("file_path", "/var/run/secrets/token"),
