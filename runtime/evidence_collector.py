...
from runtime.engine.incident_engine import IncidentManager
from runtime.engine.attack_chain import AttackChainVisualizer


class ForensicEvidenceCollector:
    def __init__(self, artifacts_dir: Optional[str] = None):
        self.artifacts_dir = artifacts_dir or os.path.join(PROJECT_ROOT, "artifacts")
        self.correlation_engine = ThreatGuardCorrelationEngine(protected_namespace="threatguard")
    ...
    def collect_live_cluster_telemetry(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if res.returncode == 0:
                ...
                # Tetragon logs
                tet_res = subprocess.run(["kubectl", "logs", "-n", "kube-system", "-l", "app.kubernetes.io/name=tetragon", "--tail=200"],
    ...
    def generate_evidence_package(
        ...
        raw_incidents = self.correlation_engine.correlate_incidents(security_events)
        incident_dicts = []
    ...
    def _render_executive_markdown(self, incidents: List[Dict[str, Any]], risk_eval: Any, event_count: int) -> str:
        lines = [
            ...
            f"**Evaluation Scope:** Namespace `threatguard`",
            "",
            "## 1. Executive Summary",
        ...
        for inc in incidents:
            tactics = ", ".join(inc.get("mitre_tactics", []))
            tech = inc.get("mitre_techniques", ["N/A"])[0]
            lines.append(f"| `{inc.get('incident_id')}` | **{inc.get('severity')}** | {inc.get('status')} | `{inc.get('namespace')}/{inc.get('pod_name')}` | {tactics} | `{tech}` |")
...
<body>
  <div class="header">
    ...
    <p>Executive Security Posture & Incident Audit Report | Scope: <code>threatguard</code></p>
  </div>
...
"""
      ...
      <tr>
        ...
        <td>{inc.get('namespace')}/{inc.get('pod_name')}</td>
        <td>{', '.join(inc.get('mitre_tactics', []))}</td>
      </tr>
"""
        html += """
    </table>
...
def main():
    ...
    for name, path in files.items():
        print(f" [+] Generated {name:<22}: {os.path.relpath(path, PROJECT_ROOT)}")
